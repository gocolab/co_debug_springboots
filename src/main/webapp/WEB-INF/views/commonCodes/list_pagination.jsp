								...
								<%-- for(int i=0;i> 9;i++){} --%>
									<c:forEach var="i" begin="${_pagination.blockStart}" end="${_pagination.blockEnd}">
										<li class="page-item"><a class="page-link"
												href="/commonCode/listPagination/${i}">${i}</a></li>
									</c:forEach>
									<li class="page-item">
								...
								<tbody>
									<c:forEach items="${resultMap.resultList}" var="resultData" varStatus="loop">
										<tr>
											...
											<td>
												<form action="/commonCode/edit/${resultData.COMMON_CODE_ID}"
													method="get">
													<button
														class="btn btn-link viewPopup">${resultData.COMMON_CODE_ID}</button>
												</form>
											</td>
											<td>
												<form action="/commonCode/editMulti/${resultData.COMMON_CODE_ID}"
													method="get">
													<button class="btn btn-link viewPopup">${resultData.COMMON_CODE_ID}
														Multi</button>
												</form>
											</td>
											...
											<td>
												<form action="/commonCode/delete/${resultData.COMMON_CODE_ID}"
													method="post">
